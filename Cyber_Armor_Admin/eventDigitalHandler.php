...
if($jsonObj->request_type == 'addEditUser'){ 
    ...
    $devices = !empty($user_data[2])?$user_data[2]:''; 
    ...
    if(empty($devices)){ 
        $err .= 'Please enter your Website devices Name.<br/>'; 
    } 
    ...
    if(!empty($user_data) && empty($err)){ 
        ...
        if(!empty($id)){ 
            ...
            $sqlQ = "UPDATE digital_forensics_request SET name=?, email=?, devices=?, requirements=?, testing_type=? WHERE request_id=?"; 
            $stmt = $conn->prepare($sqlQ); 
            $stmt->bind_param("sssssi", $name, $email, $devices, $requirements, $testing_types_json, $id); 
            $update = $stmt->execute(); 
            ...
            $sqlQ = "INSERT INTO digital_forensics_request (name, email, devices, requirements, testing_type , status) VALUES (?,?,?,?,?,?)"; 
            $stmt = $conn->prepare($sqlQ);
            $status="Request Submitted"; 
            $stmt->bind_param("ssssss", $name, $email, $devices, $requirements, $testing_types_json,$status); 
            $insert = $stmt->execute(); 
            if($insert){ 
                $output = [ 
                    'status' => 1, 
...
elseif($jsonObj->request_type == 'deleteUser'){ 
    $id = $jsonObj->user_id; 
    $sql = "DELETE FROM digital_forensics_request WHERE request_id=$id"; 
