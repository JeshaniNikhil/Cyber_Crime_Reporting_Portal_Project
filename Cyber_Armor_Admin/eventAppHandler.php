...
if($jsonObj->request_type == 'addEditUser'){ 
    ...
    $app_name = !empty($user_data[2])?$user_data[2]:''; 
    ...
    if(empty($app_name)){ 
        $err .= 'Please enter your Website app_name Name.<br/>'; 
    } 
    ...
    if(!empty($user_data) && empty($err)){ 
        ...
        if(!empty($id)){ 
            ...
            $sqlQ = "UPDATE app_request SET name=?, email=?, app_name=?, requirements=?, testing_type=? WHERE request_id=?"; 
            $stmt = $conn->prepare($sqlQ); 
            $stmt->bind_param("sssssi", $name, $email, $app_name, $requirements, $testing_types_json, $id); 
            $update = $stmt->execute(); 
            ...
            $sqlQ = "INSERT INTO app_request (name, email, app_name, requirements, testing_type , status) VALUES (?,?,?,?,?,?)"; 
            $stmt = $conn->prepare($sqlQ);
            $status="Request Submitted"; 
            $stmt->bind_param("ssssss", $name, $email, $app_name, $requirements, $testing_types_json,$status); 
            $insert = $stmt->execute(); 
            if($insert){ 
                $output = [ 
                    'status' => 1, 
...
elseif($jsonObj->request_type == 'deleteUser'){ 
    $id = $jsonObj->user_id; 
    $sql = "DELETE FROM app_request WHERE request_id=$id"; 
