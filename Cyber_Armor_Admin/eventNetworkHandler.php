...
if($jsonObj->request_type == 'addEditUser'){ 
    ...
    $network_scope = !empty($user_data[2])?$user_data[2]:''; 
    ...
    if(empty($network_scope)){ 
        $err .= 'Please enter your Website network_scope Name.<br/>'; 
    } 
    ...
    if(!empty($user_data) && empty($err)){ 
        ...
        if(!empty($id)){ 
            ...
            $sqlQ = "UPDATE network_request SET name=?, email=?, network_scope=?, requirements=?, testing_type=? WHERE request_id=?"; 
            $stmt = $conn->prepare($sqlQ); 
            $stmt->bind_param("sssssi", $name, $email, $network_scope, $requirements, $testing_types_json, $id); 
            $update = $stmt->execute(); 
            ...
            $sqlQ = "INSERT INTO network_request (name, email, network_scope, requirements, testing_type , status) VALUES (?,?,?,?,?,?)"; 
            $stmt = $conn->prepare($sqlQ);
            $status="Request Submitted"; 
            $stmt->bind_param("ssssss", $name, $email, $network_scope, $requirements, $testing_types_json,$status); 
            $insert = $stmt->execute(); 
            if($insert){ 
                $output = [ 
                    'status' => 1, 
...
elseif($jsonObj->request_type == 'deleteUser'){ 
    $id = $jsonObj->user_id; 
    $sql = "DELETE FROM network_request WHERE request_id=$id"; 
